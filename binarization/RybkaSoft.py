...
if not PYTHON_PATH:
    messagebox.showerror(
        "RybkaSoft Error Module",
        "Please provide [RYBKA_PYTHON_PATH] ENV VAR in order for GUI to boot up",
    )
    sys.exit(1)
...
A = Button(
    top,
    text="LIVE",
    command=lambda input="rybka_live": software_start(input),
    cursor="hand1",
    style="BW.TLabel",
)
B = Button(
    top,
    text="DEMO",
    command=lambda input="rybka_demo": software_start(input),
    cursor="hand1",
    style="BW.TLabel",
)
...
footer_grid.grid(column=0, columnspan=3, row=5, pady=5)
...
link.grid(column=0, row=6, pady=5, padx=(5, 10))
link.bind("<Button-1>", lambda e: callback("https://silviu_space.gitlab.io/rybka"))

# Copyright + Email
footer_email = Label(
    text="Contact:\nsilviumuraru90@yahoo.com",
    style="Heading.TLabel",
    foreground="DarkOrange4",
    background="black",
    font=("Courier", 11, "bold"),
)
footer_email.grid(column=1, row=6, pady=5, padx=15)

# Build ID
strvar = IntVar(top, name=build_id)
footer_release = Label(
    text=strvar,
    style="Heading.TLabel",
    foreground="DarkOrange4",
    background="black",
...
footer_release.grid(column=2, row=6, padx=(10, 5), pady=5)
