...
def start_cmds_template(update, context, module):
    try:
        ...
        if psutil.pid_exists(pID) and "python" in psutil.Process(pID).name():
            ...
            update.message.reply_text(
                f"🚀 Starting bot via `{module}` module!\nPlease wait..."
            )
            try:
...
def stop_software_command(update, context):
    try:
        ...
        if psutil.pid_exists(pID) and "python" in psutil.Process(pID).name():
            ...
            update.message.reply_text(
                f"🪓 Killing the process [pID:{str(pID)}]!\nPlease wait..."
            )
            if (
                psutil.pid_exists(restarter_pID)
                and "python" in psutil.Process(pID).name()
            ):
                restarter_run = True
                update.message.reply_text(
                    f"🪓 Killing a secondary process [pID:{str(restarter_pID)}]!\nPlease wait..."
                )
            else:
                restarter_run = False
            try:
            ...
            update.message.reply_text(
                f"💤 Bot is already stopped at this moment. Last known [pID:{str(pID)}]"
            )
